...
pinit(Ctlr *c, uint queue, Sring *r)
{
	...
	le32put(u.u, Pciwaddrh(r->r));
	prwrite32(c, queue + Paddrh, u.l);
	le32put(u.u, Pciwaddrl(r->r));
...
tproc(void *v)
{
	...
	for(;;){
		...
		if(Pciwaddrh(b->rp) != 0){
			...
			t->ctl = 0;
			t->op = Oaddr64 | Hw;
			le32put(t->status, Pciwaddrh(b->rp));
		}
		t = getslot(r, k);
		c->tbring[t - r->r] = b;
		le32put(t->status, Pciwaddrl(b->rp));
		le16put(t->l, BLEN(b));
		t->op = Opkt | Hw;
		t->ctl = Eop;
		sfence();
...
replenish(Ether *e, Ctlr *c)
{
	...
	for(n = 0; n < lim; n++){
		...
		t = tab[0];
		if(is64()){
			le32put(t->status, Pciwaddrh(b->wp));
			t->ctl = 0;
			t->op = Oaddr64 | Hw;
			t = tab[1];
		}
		...
		le32put(t->status, Pciwaddrl(b->wp));
		le16put(t->l, c->rbsz);
		t->ctl = 0;
		t->op = Opkt | Hw;
	}
